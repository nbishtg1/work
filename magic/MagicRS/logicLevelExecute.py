...
class algoLogic:
    ...
    openPnl = pd.DataFrame(
            columns=['Key','Symbol', 'EntryPrice', 'CurrentPrice',
                    'Quantity',
                    'PositionStatus', 'Pnl','hedge','count','straddle'])
    ...
    def OHLCDataFetch(self,collectionMain):
        ...
        # print(dataCountMain)
        OHLCMain = pd.DataFrame(collectionMain.find().skip(dataCountMain - 500))
    ...
    def entryOrder(self, data, symbol,quantity, entrySide,hedge,count,straddle):
        ...
        self.levelAdder(entryPrice, symbol,quantity,positionSide,hedge,count,straddle)

        return entryPrice
                
    
    def levelAdder(self, entryPrice,symbol,quantity, positionSide,hedge,count,straddle):

        self.openPnl.loc[len(self.openPnl)] = [datetime.datetime.fromtimestamp(self.timeData)]+[symbol] + [entryPrice] \
                                        + [entryPrice] +[quantity]+[positionSide] + [0] + [hedge] + [count] + [straddle]
       
        try:self.openPnl.to_csv(f'openPosition.csv')
        except:pass           
    ...
    def mainLogic(self, indexName, baseSym, strikeDist, isLive, algoName, algoConn, pingConn, clientDataInfo, clientInfo, clientRedis, configReader, configReaderIP ):
        ...
        try:   
            ...
            #access collectin(for candle data) using liveMarket Database(i.e clientData)
            ...
            while not done:
                ...
                if humanTime >= datetime.datetime(humanTime.year,humanTime.month,humanTime.day,9,15,20):
                    ...
                    straddlecount=Data["straddlecount"]
                    signal=Data["signal"]
                    count=Data["count"]    
            ...
            while proceed:
                ...
                # -------------------------- Start Time ----------------------------------------
                if humanTime.time() >= datetime.time(9, 15,40) and humanTime.time() <= algoEndTime:
                    timeData = time.time()
                    self.timeData = timeData
                    ...
                    if OHLCMain.index[-1] >  lastEnt:
                        ...
                        if humanTime.time()<= datetime.time(10,0,0):
                            if not self.openPnl.empty:    
                                for index, row in self.openPnl.iterrows():                                                          
                                    if row["PositionStatus"]==1 and (row["hedge"]=="overnight hedge"):            
                                        ...
                                        infoMessage(mongo = client, redis = pingConn, algoName = algoName, message=f'OVERNIGHT HEDGE {row["Symbol"]} closed')
                                        
                                        print(f"{humanTime}...........overnight hedge closed {row['Symbol']} @{row['CurrentPrice']}")
                                        logData(f"{humanTime}...........overnight hedge closed {row['Symbol']} @{row['CurrentPrice']}")         
                                self.openPnl.reset_index(inplace=True,drop=True)
                            
                        
                        if not self.openPnl.empty:     
                            df= self.openPnl[(self.openPnl["straddle"]==1)  &  (self.openPnl["PositionStatus"]==-1)]   
                            straddlecount = df["count"].nunique() 
                            if not df.empty:
                                lt1=[]
                                lt2=[]
                                closeHedge=0
                                df2 = df.groupby(['count']).sum() 
                                df2= df2.reset_index()
                                
                                for index , row in df2.iterrows():        
                                    if row["CurrentPrice"]>1.6*row["EntryPrice"]:
                                        lt1.append(row["count"])    
                                        straddlecount-=1
                                        if straddlecount>=1:
                                            closeHedge+=1                
                                    elif row["CurrentPrice"]<0.4*row["EntryPrice"]: 
                                        if getCurrentExpiry()!= self.date_expformat(timeData):   
                                            lt2.append(row["count"])
                                            straddlecount-=1  
                                            if straddlecount>=1:
                                                closeHedge+=1
                                        elif getCurrentExpiry()== self.date_expformat(timeData):  
                                            if row["CurrentPrice"]<0.2*row["EntryPrice"]:   
                                                lt2.append(row["count"])
                                                straddlecount-=1  
                                                if straddlecount>=1:
                                                    closeHedge+=1
                                        ...
                                        if row["PositionStatus"]==-1:
                                            if row["count"] in lt1:                             
                                                self.closedPnl.loc[len(self.closedPnl)] = [row['Key']] + [datetime.datetime.fromtimestamp(timeData)] +[row['Symbol']]+\
                                                                                                    ...
                                                                                                    [row['CurrentPrice']]+\
                                                                                                    [row['Quantity']]+[row['PositionStatus']]+\
                                                                                                    [0]+["straddle stoploss hit"] 
                                                self.openPnl.drop(index,inplace=True)   
                                                self.exitOrder(data=data , symbol=row['Symbol'] , quantity=row['Quantity'] , PositionStatus=row['PositionStatus'])      
                                                infoMessage(mongo = client, redis = pingConn, algoName = algoName, message=f'STRADDLE {row["Symbol"]} CLOSED , STOPLOSS HIT')           
                                                print(f"{humanTime}...........{row['count']} stradde stoploss hit, closed {row['Symbol']} positions @{row['CurrentPrice']}")
                                                logData(f"{humanTime} {row['count']} straddle stoploss hit, closed {row['Symbol']} positions @{row['CurrentPrice']}")
                                    self.openPnl.reset_index(inplace=True,drop=True)
                                
                                if lt2!=[]:
                                    # straddle_count-=int(len(lt2))
                                    logData(df2)
                                    for index , row in self.openPnl.iterrows():                           
                                        if row["PositionStatus"]==-1:    
                                            if row["count"] in lt2:                             
                                                self.closedPnl.loc[len(self.closedPnl)] = [row['Key']] + [datetime.datetime.fromtimestamp(timeData)] +[row['Symbol']]+\
                                                                                                    ...
                                                                                                    [row['CurrentPrice']]+\
                                                                                                    [row['Quantity']]+[row['PositionStatus']]+\
                                                                                                    [0]+["straddle target reached"] 
                                                self.openPnl.drop(index,inplace=True)  
                                                self.exitOrder(data=data , symbol=row['Symbol'] , quantity=row['Quantity'] , PositionStatus=row['PositionStatus'])                  
                                                infoMessage(mongo = client, redis = pingConn, algoName = algoName, message=f'STRADDLE {row["Symbol"]} CLOSED , TARGET HIT')
                                                print(f"{humanTime}...........{row['count']} stradde target reached, closed {row['Symbol']} positions @{row['CurrentPrice']}")
                                                logData(f"{humanTime} {row['count']} straddle target reached, closed {row['Symbol']} positions @{row['CurrentPrice']}")
                                    self.openPnl.reset_index(inplace=True,drop=True)
                                ...
                                if closeHedge!=0:
                                    ...
                                    if not self.openPnl.empty:
                                        for index, row in self.openPnl.iterrows():
                                            z=False
                                            if ("CE" in row["Symbol"]) and row["PositionStatus"]==1 and (row["hedge"]=="margin hedge") and row["straddle"]==1:
                                                if CEh < closeHedge:
                                                    CEh+=1
                                                    z=True
                                            if ("PE" in row["Symbol"]) and row["PositionStatus"]==1 and (row["hedge"]=="margin hedge") and row["straddle"]==1:
                                                if PEh < closeHedge:
                                                    PEh+=1
                                                    z=True 
                                            if z:
                                                ...
                                                self.openPnl.drop(index,inplace=True)
                                                self.exitOrder(data=data , symbol=row['Symbol'] , quantity=row['Quantity'] , PositionStatus=row['PositionStatus'])
                                                infoMessage(mongo = client, redis = pingConn, algoName = algoName, message=f'MARGIN HEDGE {row["Symbol"]} closed')                                  
                        ...
                        if not self.openPnl.empty:  
                                ...
                                if row["PositionStatus"]==-1:
                                    if row["straddle"]==0:
                                        if row["CurrentPrice"] < (target * row['EntryPrice']): 
                                            straddlecount+=1   
                                            if straddlecount>1:
                                                hedgestraddle.append(row['EntryPrice']*target) 
                                                
                                            remainder = indexPrice%strikeDist
                                            atmStrike = indexPrice -remainder if remainder<=(strikeDist/2) \
                                                                        else (indexPrice -remainder + strikeDist)
                                            atmStrike=int(atmStrike)
                                            
                                            strike= row["Symbol"][16:-2]
                                            try:
                                                strike=int(strike)     
                                            except:
                                                strike=atmStrike              
                                            ...
                                            if "CE" in row["Symbol"]:
                                                if strike<atmStrike:
                                                    itm=True   
                                            elif "PE" in row["Symbol"]:
                                                if strike>atmStrike:
                                                    itm=True
                                            
                                            if not itm:                                                
                                                if "CE" in row["Symbol"]:
                                                    straddle.append(["PE",row['EntryPrice'],row["count"],row["Symbol"][:16]])
                                                    if CE_count>=2:
                                                        closeHedgeCE=True                                                                   
                                                if "PE" in row["Symbol"]: 
                                                    straddle.append(["CE",row['EntryPrice'],row["count"],row["Symbol"][:16]]) 
                                                    if PE_count>=2:
                                                        closeHedgePE=True 
                                                ...
                                                if row["CurrentPrice"]< (target-target*0.15)*row['EntryPrice']:
                                                    self.openPnl.drop(index,inplace=True) 
                                                    self.exitOrder(data=data , symbol=row['Symbol'] , quantity=row['Quantity'] , PositionStatus=row['PositionStatus'])
                                                    ...
                                                    self.openPnl.at[index,"straddle"]=1
                                                    self.openPnl.at[index,"EntryPrice"]=self.openPnl.at[index,"CurrentPrice"]
                                                    self.openPnl.at[index,"Key"]=humanTime
                                                    infoMessage(mongo = client, redis = pingConn, algoName = algoName, message=f'TARGET HIT ,CONVERT {row["Symbol"]} TO STRADDLE')                                  
                                                    print(f"{humanTime}...........target hit, convert {row['Symbol']} @{row['CurrentPrice']} to Straddle")
                                                    logData(f"{humanTime}..........target hit, convert {row['Symbol']} @{row['CurrentPrice']} to Straddle") 
                                                    self.openPnl.at[index,"straddle"]=1
                                            
                                            elif itm:
                                                atmstraddle.append([row["count"],row["Symbol"][:16]])
                                                if "CE" in row["Symbol"]:
                                                    if CE_count>=2:
                                                        closeHedgeCE=True                                                                   
                                                if "PE" in row["Symbol"]: 
                                        ...
                                        if row["CurrentPrice"] > (stoploss * row['EntryPrice']):
                                            if "CE" in row["Symbol"]:
                                                if CE_count>=2:
                                                    closeHedgeCE=True  
                                            if "PE" in row["Symbol"]:
                            ...
                            if hedgestraddle != []:
                                for i in hedgestraddle:                               
                                    ...
                                    if callHedge!="NotFound":
                                        ...
                                        self.entryOrder(data=data,symbol=callHedge, quantity=quantity,entrySide="BUY",hedge="margin hedge",count=0,straddle=1)         
                                        ...
                                        infoMessage(mongo = client, redis = pingConn, algoName = algoName, message=f'BUY MARGIN HEDGE {callHedge}')
                                    ...
                                    if putHedge!="NotFound":
                                        ...
                                        self.entryOrder(data=data,symbol=putHedge, quantity=quantity,entrySide="BUY",hedge="margin hedge",count=0,straddle=1)         
                                        ...
                                        infoMessage(mongo = client, redis = pingConn, algoName = algoName, message=f'BUY MARGIN HEDGE {putHedge}')
                                time.sleep(2)        
                            
                            if atmstraddle!=[]:
                                for i in atmstraddle:
                                    callSym = self.getCall(indexPrice=indexPrice, strikeDist=strikeDist, symWithExpiry=i[1],otmFactor=0)
                                    putSym = self.getPut(indexPrice=indexPrice, strikeDist=strikeDist, symWithExpiry=i[1],otmFactor=0)
                                    ...
                                    self.entryOrder(data=data,symbol=callSym, quantity=quantity,entrySide="SELL",hedge=0,count=i[0],straddle=1)
                                    print(f'{humanTime} ..................sold {callSym}  @{data[self.idMap[callSym]]}')
                                    logData(f'{humanTime} .................sold {callSym}  @{data[self.idMap[callSym]]}')
                                    infoMessage(mongo = client, redis = pingConn, algoName = algoName, message=f'NEW ENTRY {callSym} ATM STRADDLE SELL')
                                                    
                                    self.entryOrder(data=data,symbol=putSym, quantity=quantity,entrySide="SELL",hedge=0,count=i[0],straddle=1)
                                    print(f'{humanTime} ..................sold {putSym}  @{data[self.idMap[putSym]]}')
                                    logData(f'{humanTime} .................sold {putSym}  @{data[self.idMap[putSym]]}')
                                    infoMessage(mongo = client, redis = pingConn, algoName = algoName, message=f'NEW ENTRY {putSym} ATM STRADDLE SELL')
                                    
                            if straddle!=[]:
                                for i in straddle:
                                    Sym=getSymbyPrice(symSide=i[0],baseSym=i[3],priceReq=i[1]*target*1.1,lesser_grater="lesser",configReader=configReaderIP)
                                    sock, data, self.idMap, self.symListConn = reconnect(sock, self.idMap, [Sym]) 
                                    self.entryOrder(data=data,symbol=Sym, quantity=quantity,entrySide="SELL",hedge=0,count=i[2],straddle=1)
                                    print(f'{humanTime} ..................sold {Sym}  @{data[self.idMap[Sym]]} straddle')
                                    logData(f'{humanTime} .................sold {Sym}  @{data[self.idMap[Sym]]} straddle')
                                    infoMessage(mongo = client, redis = pingConn, algoName = algoName, message=f'NEW ENTRY {Sym} STRADDLE SELL')    
                            ...
                            if closeHedgePE or closeHedgeCE:
                                ...
                                if not self.openPnl.empty:  
                                    for index, row in self.openPnl.iterrows():
                                        z=False
                                        if closeHedgeCE and ("CE" in row["Symbol"]) and (row["PositionStatus"]==1) and (row["hedge"]=="margin hedge") and row["straddle"]==0:
                                            z=True
                                        if closeHedgePE and ("PE" in row["Symbol"]) and row["PositionStatus"]==1 and (row["hedge"]=="margin hedge") and row["straddle"]==0:
                                            z=True 
                                        if z:
                                            self.closedPnl.loc[len(self.closedPnl)] = [row['Key']] + [datetime.datetime.fromtimestamp(timeData)] +[row['Symbol']]+\
                                                                                        [row['EntryPrice']]+\
                                                                                        [row['CurrentPrice']]+\
                                                                                        [row['Quantity']]+[row['PositionStatus']]+\
                                                                                        [0]+['close margin hedge']                             
                                            self.openPnl.drop(index,inplace=True)
                                            self.exitOrder(data=data , symbol=row['Symbol'] , quantity=row['Quantity'] , PositionStatus=row['PositionStatus'])
                                            infoMessage(mongo = client, redis = pingConn, algoName = algoName, message=f'MARGIN HEDGE {row["Symbol"]} closed')                                  
                                            print(f"{humanTime}...........margin hedge closed {row['Symbol']} @{row['CurrentPrice']}")
                                            logData(f"{humanTime}..........margin hedge closed {row['Symbol']} @{row['CurrentPrice']}")
                                    self.openPnl.reset_index(inplace=True,drop=True)  
                        ...
                        if trade:     
                            ...
                            if signal=="short":
                                if CE_count<2:
                                    ...
                                    callSym = self.getCall(indexPrice=indexPrice, strikeDist=strikeDist, symWithExpiry=symWithExpiry,otmFactor=otmFactor)
                                    sock, data, self.idMap, self.symListConn = reconnect(sock, self.idMap, [callSym])
                                    if priceupperlimit >= data[self.idMap[callSym]]>=pricelowerlimit:
                                        pass
                                    elif priceupperlimit < data[self.idMap[callSym]]:
                                        callSym=getSymbyPrice(symSide="CE",baseSym=symWithExpiry,priceReq=priceupperlimit,lesser_grater="lesser",configReader=configReaderIP)
                                        sock, data, self.idMap, self.symListConn = reconnect(sock, self.idMap, [callSym])
                                    elif pricelowerlimit > data[self.idMap[callSym]]:
                                    ...
                                    if CE_count==1:
                                        ...
                                        if callHedge!="NotFound":
                                            sock, data, self.idMap, self.symListConn = reconnect(sock, self.idMap, [callHedge])
                                            self.entryOrder(data=data,symbol=callHedge, quantity=quantity,entrySide="BUY",hedge="margin hedge",count=0,straddle=0)         
                                            print(f'{humanTime} ..................buy Hedge {callHedge}  @{data[self.idMap[callHedge]]}')
                                            logData(f'{humanTime} .................buy Hedge {callHedge}  @{data[self.idMap[callHedge]]}')
                                    ...
                                    data = dataFetcher(sock, self.symListConn)
                                    self.entryOrder(data=data,symbol=callSym, quantity=quantity,entrySide="SELL",hedge=0,count=count,straddle=0)
                                    print(f'{humanTime} ..................sold {callSym}  @{data[self.idMap[callSym]]}')
                                    logData(f'{humanTime} .................sold {callSym}  @{data[self.idMap[callSym]]}')
                                    infoMessage(mongo = client, redis = pingConn, algoName = algoName, message=f'NEW ENTRY {callSym} SELL')
                                    CE_count+=1
                                    count+=1
                                    logData(f"CE_count:{CE_count}")
                                    # trade=False
                            ...
                            if signal=="long":
                                if PE_count<2:
                                    ...
                                    putSym = self.getPut(indexPrice=indexPrice, strikeDist=strikeDist, symWithExpiry=symWithExpiry,otmFactor=otmFactor)
                                    sock, data, self.idMap, self.symListConn = reconnect(sock, self.idMap, [putSym])
                                    if priceupperlimit >= data[self.idMap[putSym]]>=pricelowerlimit:
                                        pass
                                    elif priceupperlimit < data[self.idMap[putSym]]:
                                        putSym=getSymbyPrice(symSide="PE",baseSym=symWithExpiry,priceReq=priceupperlimit,lesser_grater="lesser",configReader=configReaderIP)
                                        sock, data, self.idMap, self.symListConn = reconnect(sock, self.idMap, [putSym])
                                    elif pricelowerlimit > data[self.idMap[putSym]]:
                                    ...
                                    if PE_count==1:
                                        ...
                                        if putHedge!="NotFound":
                                            sock, data, self.idMap, self.symListConn = reconnect(sock, self.idMap, [putHedge])
                                            self.entryOrder(data=data,symbol=putHedge, quantity=quantity,entrySide="BUY",hedge="margin hedge",count=0,straddle=0)         
                                            print(f'{humanTime} ..................buy Hedge {putHedge}  @{data[self.idMap[putHedge]]}')
                                            logData(f'{humanTime} .................buy Hedge {putHedge}  @{data[self.idMap[putHedge]]}')
                                    ...
                                    data = dataFetcher(sock, self.symListConn)    
                                    self.entryOrder(data=data,symbol=putSym, quantity=quantity,entrySide="SELL",hedge=0,count=count,straddle=0)
                                    print(f'{humanTime} ..................sold {putSym}  @{data[self.idMap[putSym]]}')
                                    logData(f'{humanTime} .................sold {putSym}  @{data[self.idMap[putSym]]}')
                                    infoMessage(mongo = client, redis = pingConn, algoName = algoName, message=f'NEW ENTRY {putSym} SELL')
                                    PE_count+=1
                                    count+=1
                                    logData(f"PE_count:{PE_count}")
                                    # trade=False
                            trade=False                                                                                                    
                    
                        if candle.time()>=datetime.time(15, 24, 0): 
                            ...
                            dayCloseEpoch= int(datetime.datetime(humanTime.year, humanTime.month, humanTime.day,15,23,0).timestamp())
                            dayClose = OHLCMain['Close'][dayCloseEpoch]  
                            
                            if dayClose>=dayOpen:
                                signal="short" 
                            elif dayClose<dayOpen:
                                signal="long" 
                            
                            logData(f"dayOpen:{dayOpen} , dayClose:{dayClose} , signal:{signal}") 
                            ...
                            if not self.openPnl.empty:
                                for index, row in self.openPnl.iterrows():
                                    if ("CE" in row["Symbol"]) and (row["PositionStatus"]==-1):
                                        CE_value+=row["EntryPrice"]
                                    if ("PE" in row["Symbol"]) and (row["PositionStatus"]==-1):
                                        PE_ct+=1
                                        PE_value+=(row["EntryPrice"]-row["CurrentPrice"])
                            
                            try: 
                                df=pd.read_csv("../magic/magicTrendp5/openPosition.csv",index_col=0)
                                if not df.empty:
                            ...
                            value= CE_value+PE_value                 
                            otm=z+value
                            logData(f"combined hedge buffer value(CE+PE):{value}")  
                            logData(f"open PE positions:{PE_ct}")                 
                            if PE_ct!=0:
                                otm=otm/PE_ct    
                                remainder = otm%strikeDist
                                otm = otm -remainder if remainder<=(strikeDist/2) \
                                                    else (otm -remainder + strikeDist) 
                                otm= int(otm)
                                logData(f"otmPerPosition:{otm}")
                                if self.date_expformat(timeData)!=getCurrentExpiry():
                                    expiry = getCurrentExpiry()
                                if self.date_expformat(timeData)==getCurrentExpiry():
                                    expiry = getNextExpiry()
                                ...
                                try:
                                    logData(f"1:{putSym}")
                                    sock, data, self.idMap, self.symListConn = reconnect(sock, self.idMap, [putSym]) 
                                    self.entryOrder(data=data,symbol=putSym, quantity=quantity*PE_ct, entrySide="BUY",hedge="overnight hedge",count=0,straddle=0)   
                                    print(f"{humanTime} buy hedge {putSym} ")
                                    logData(f'{humanTime}-------------hedge {putSym} long position') 
                                    infoMessage(mongo = client, redis = pingConn, algoName = algoName, message=f'BUY OVERNIGHT HEDGE {putSym}')                 
                                except Exception as e:
                                    logData(e)
                                    if otm>1000:
                                        logData("2")
                                        for j in range(otm-strikeDist,900,-strikeDist):
                                            logData(f"j:{j}")
                                            putSym=self.getPut(indexPrice=indexPrice,strikeDist=strikeDist,symWithExpiry=symWithExpiry,otmFactor=j)
                                            try:
                                                sock, data, self.idMap, self.symListConn = reconnect(sock, self.idMap, [putSym]) 
                                                self.entryOrder(data=data,symbol=putSym, quantity=quantity*PE_ct,entrySide="BUY",hedge="overnight hedge",count=0,straddle=0)   
                                                logData(f"hedge captured at {j} otm")
                                                print(f"{humanTime} buy hedge {putSym} ")
                                                logData(f'{humanTime}-------------hedge {putSym} long position') 
                                                infoMessage(mongo = client, redis = pingConn, algoName = algoName, message=f'BUY OVERNIGHT HEDGE {putSym}')                        
                                                break
                                            except:
                                                logData(f"data not found for {putSym}")
                        ...
                        Data["count"]= count
                        Data["trade"]=trade
                        Data["straddlecount"]=straddlecount
                        Data["signal"]=signal
